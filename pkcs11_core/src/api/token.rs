...
use crate::{
    ...
    data::{BACKEND, DEVICE, EVENTS_MANAGER},
...
pub extern "C" fn C_GetSlotInfo(
    ...
    // Use backend abstraction to get device info and system state
    let Some(backend_guard) = BACKEND.load_full() else {
        error!("Backend not initialized");
        return cryptoki_sys::CKR_CRYPTOKI_NOT_INITIALIZED;
    };

    let backend = match backend_guard.lock() {
        Ok(backend) => backend,
        Err(_) => {
            error!("Failed to acquire backend lock");
            return cryptoki_sys::CKR_DEVICE_ERROR;
        }
    };

    let slot_id = crate::backend::SlotId(slotID as u32);

    // Get device info from backend
    let info = match backend.get_device_info(slot_id) {
        Ok(info) => info,
        Err(e) => {
            trace!("Error getting device info: {e:?}");
    ...
    // Get system state from backend
    let system_state = match backend.get_system_state(slot_id) {
        Ok(state) => state,
        Err(e) => {
            trace!("Error getting system state: {e:?}");
            crate::backend::SystemState::Unknown
        }
    };

    if system_state == crate::backend::SystemState::Operational {
        flags |= cryptoki_sys::CKF_TOKEN_PRESENT;
    }

    let slot_info: CK_SLOT_INFO = CK_SLOT_INFO {
...
pub extern "C" fn C_GetTokenInfo(
    ...
    // Use backend abstraction to get device info
    let Some(backend_guard) = BACKEND.load_full() else {
        error!("Backend not initialized");
        return cryptoki_sys::CKR_CRYPTOKI_NOT_INITIALIZED;
    };
    ...
    // Use device info from backend
    if let Some(device_id) = &info.device_id {
        serial_number = device_id.clone();
    }
    if let Some(hw_version) = &info.hardware_version {
        hardware_version = version_struct_from_str(hw_version.clone());
    }
    if let Some(sw_version) = &info.software_version {
        // The PKCS11 firmware version actually corresponds to the NetHSM software version
        firmware_version = version_struct_from_str(sw_version.clone());
    }
