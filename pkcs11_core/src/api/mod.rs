...
use crate::{
    ...
    data::{self, BACKEND, DEVICE, EVENTS_MANAGER, THREADS_ALLOWED, TOKENS_STATE},
    defs,
    utils::padded_str,
};
...
pub extern "C" fn C_Finalize(pReserved: CK_VOID_PTR) -> CK_RV {
    ...
    if let Some(backend_mutex) = BACKEND.load_full() {
        if let Ok(mut backend) = backend_mutex.lock() {
            if let Err(err) = backend.finalize() {
                error!("Failed to finalize backend: {err}");
                // Continue with cleanup even if backend finalization fails
            }
        }
    }
    BACKEND.store(None);
